...
int binary = 0, type = 0, debug_mode = 2, window = 5, min_count = 5, num_threads = 12, min_reduce = 1;
int *vocab_hash;
long long vocab_max_size = 1000, vocab_size = 0, layer1_size = 100;
long long train_words = 0, word_count_actual = 0, iter = 5, file_size = 0, classes = 0;
real alpha = 0.025, starting_alpha, sample = 1e-3;
real *syn0, *syn1, *syn1neg, *syn1nce, *expTable, *syn1ds, *syn1neg0, *syn1neg1;
clock_t start;
...
void InitNet() {
  ...
  if (ds) {
  	a = posix_memalign((void **)&syn1ds, 128, (long long)vocab_size * layer1_size * sizeof(real));
    if (syn1ds == NULL) {printf("Memory allocation failed\n"); exit(1);}
    
    for (a = 0; a < vocab_size; a++) for (b = 0; b < layer1_size; b++)
     syn1ds[a * layer1_size + b] = (((next_random & 0xFFFF) / (real)65536) - 0.5) / layer1_size;
  }
  if (negative>0) {
  ...
  if (type>0) {
  	a = posix_memalign((void **)&syn1neg0, 128, (long long)vocab_size * layer1_size * sizeof(real));
    if (syn1neg0 == NULL) {printf("Memory allocation failed\n"); exit(1);}
    a = posix_memalign((void **)&syn1neg1, 128, (long long)vocab_size * layer1_size * sizeof(real));
    if (syn1neg1 == NULL) {printf("Memory allocation failed\n"); exit(1);}
    
    for (a = 0; a < vocab_size; a++) for (b = 0; b < layer1_size; b++)
     syn1neg0[a * layer1_size + b] = 0;
    for (a = 0; a < vocab_size; a++) for (b = 0; b < layer1_size; b++)
     syn1neg1[a * layer1_size + b] = 0;
  }
  if (nce>0) {
...
void *TrainModelThread(void *id) {
  ...
  long long l1, l2, c, dlr, target, label, local_iter = iter;
  ...
  while (1) {
    ...
    if (type == 0) { //train directional skip-gram
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
        ...
        // ds
        if (ds > 0) {
          dlr = sentence_position - c;
          l2 = word * layer1_size;
          f = 0;
          for (c = 0; c < layer1_size; c++) f += syn0[c + l1] * syn1ds[c + l2];
          label = 1;
          if(dlr < 0)
            label = 0;
          ...
          for (c = 0; c < layer1_size; c++) neu1e[c] += g * syn1ds[c + l2];
          for (c = 0; c < layer1_size; c++) syn1ds[c + l2] += g * syn0[c + l1];
        }
        // NEGATIVE SAMPLING
    ...
    } else if(type==1) {  //train simple structured skip-gram (SSSG)
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
        c = sentence_position - window + a;
        if (c < 0) continue;
        if (c >= sentence_length) continue;
        ...
        // NEGATIVE SAMPLING
          ...
          if(dlr < 0)
            for (c = 0; c < layer1_size; c++) f += syn0[c + l1] * syn1neg0[c + l2];
          else
          	for (c = 0; c < layer1_size; c++) f += syn0[c + l1] * syn1neg1[c + l2];
          ...
          if(dlr < 0) {
            for (c = 0; c < layer1_size; c++) neu1e[c] += g * syn1neg0[c + l2];
            for (c = 0; c < layer1_size; c++) syn1neg0[c + l2] += g * syn0[c + l1];
            if (cap == 1) for (c = 0; c < layer1_size; c++) capParam(syn1neg0, c + l2);
          } else {
            for (c = 0; c < layer1_size; c++) neu1e[c] += g * syn1neg1[c + l2];
            for (c = 0; c < layer1_size; c++) syn1neg1[c + l2] += g * syn0[c + l1];
            if (cap == 1) for (c = 0; c < layer1_size; c++) capParam(syn1neg1, c + l2);
          }
        }
        //Noise Contrastive Estimation
        if (nce > 0) for (d = 0; d < nce + 1; d++) {
...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    ...
    printf("\t-type <int>\n");
    printf("\t\tType of embeddings (0 for directional skip-gram, 1 for simple structured skip-gram)\n");
    printf("\t-cap <int>\n");
    printf("\t\tlimit the parameter values to the range [-50, 50]; default is 0 (off)\n");
    printf("\nExamples:\n");
    printf("./dsg -train data.txt -output vec.txt -size 200 -window 5 -sample 1e-4 -negative 5 -hs 0 -binary 0 -type 0 -iter 3\n\n");
    return 0;
  }
  output_file[0] = 0;
  save_vocab_file[0] = 0;
